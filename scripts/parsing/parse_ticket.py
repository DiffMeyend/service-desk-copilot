...
if __package__ in (None, ""):
    # Allow running the parser directly via `python scripts/parsing/parse_ticket.py`
...
RE_STREET_ADDRESS = re.compile(
    ...
    r"Parkway|Pkwy|Way|Circle|Cir|Highway|Hwy)\b\.?",
    re.I,
)
...
def scrub_text(text: str) -> str:
    if not text:
        return text
    # Protect ticket IDs from redaction (they can look like phone numbers).
    ticket_ids = []

    def _mask(match: re.Match[str]) -> str:
        ticket_ids.append(match.group(0))
        return f"__TID_{len(ticket_ids) - 1}__"

    scrubbed = RE_TICKET_ID.sub(_mask, text)
    for pattern in SENSITIVE_PATTERNS:
        scrubbed = pattern.sub("[REDACTED]", scrubbed)
    scrubbed = RE_PHONE.sub("[REDACTED_PHONE]", scrubbed)
    scrubbed = RE_STREET_ADDRESS.sub("[REDACTED_ADDRESS]", scrubbed)
    for i, ticket_id in enumerate(ticket_ids):
        scrubbed = scrubbed.replace(f"__TID_{i}__", ticket_id)
    return scrubbed
        ...
        if line == "LOG_RESULT" or line.startswith("LOG_RESULT "):
            ...
            if json_part.startswith("{"):
                ...
                if isinstance(payload, dict) and payload.get("command_id"):
                    ...
                    i += 1
                    continue
            # LOG_RESULT block with fenced output
            j = i + 1
            ...
            if j < len(lines) and lines[j].strip().startswith("```"):
                ...
                output = "\n".join(block).strip()
                if output:
                    evidence.append({"command_id": f"raw_log_{raw_counter}", "output": output})
                    raw_counter += 1
                i = j + 1
                continue
...
def extract_inline_field(text: str, label: str) -> Optional[str]:
    ...
    Tightened to:
    - anchor to line starts (ignoring whitespace)
    - stop when encountering blank lines, known section headings, or other inline labels
    - support short multi-line responses while avoiding miscaptures into later sections
    """
    ...
    for raw_line in text.splitlines():
        ...
        # allow short multi-line responses but avoid runaway captures
    ...
    payload: Dict[str, object] = {
        ...
        "environment": {
            "target_device": {
                ...
                "asset_tag": device.get("asset_tag") or first_asset.get("asset_id", ""),
                "serial_number": device.get("serial") or first_asset.get("serial", ""),
            },
            "user_context": {
